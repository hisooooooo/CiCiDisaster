...
public class GrappleRope : MonoBehaviour
{
    ...
    void LinePointToFirePoint()
    {
        ...
        {
            m_lineRenderer.SetPosition(i, player.transform.position);
        }
    }
    ...
    void DrawRopeWaves()
    {
        for (int i = 0; i < percision; i++)
        {
            ...
            Vector2 targetPosition = Vector2.Lerp(player.transform.position, grapplingGun.grapplePoint, delta) + offset;
            Vector2 currentPosition = Vector2.Lerp(player.transform.position, targetPosition, ropeLaunchSpeedCurve.Evaluate(moveTime) * ropeLaunchSpeedMultiplayer);
